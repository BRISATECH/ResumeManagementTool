...
class userinfo:
    def __init__(self):
         ...
         self.cur = django.db.connections['rmtdb'].cursor()

         # SQLITE COMMENT
         
######         self.cur = mdb.connect("sqlit3_LOCAL.db")
######
######         
######         self.keyVarible="$%^)*)*hgfyv     ghfdv587&%$^%#^$#@@G B78967987908 $#^$#^$# #^^%s([YINTBIUTIUYTKJNp0klghbb  k879870HGFVJYURr ut7 y i"
         pass
...
####Lavita##################################################################
    ...
    def submitSignIn(self,username,password):
        
        
        try:
            cursor = self.cur.execute("SELECT username,password from User_authentication ")
            ...
            for row in rows :
                ...
                if(row[0]==str(username) and row[1]==str(password)):
                    ...
                    return flag
##                else:
##                     flag="is username or password correct%s"
##            return flag
...
##########Lavita##################################################################################33
    ...
    def database_fetch(self,uname,pwd):
        ...
        cursore = self.cur.execute("SELECT Username from User_authentication where username = (%s)", [uname])

# snehali new code
        userrow = self.cur.fetchone()
...
##        userrow = cursore.fetchone()
        ...
        while(1):
                ...
                l=l-1
        print 'var',var
        cursore = self.cur.execute("SELECT Tag_value from User_authentication where username = (%s)", [uname])
        # snehali new code
        Reference_ID = self.cur.fetchone()
##        Reference_ID = cursore.fetchone()

        if var:
            print "#####################IM i n condition########################################"
            cursore=self.cur.execute("SELECT Password from User_authentication where password = (%s)", [pwd])
             # snehali new code
            passwcorrect = self.cur.fetchone()
...
##        self.cur.commit()
    ...
    def chanpwd(self,Old_PWD,New_PWD,Cnfrm_PWD,Username):
        ...
        cursore = self.cur.execute("SELECT Username from User_authentication where username = (%s)", [user_name])
...
##        userrow = cursore.fetchone()
      ...
      #  Reference_ID = cursore.fetchone()
        ...
        if var:
            print "#####################IM i n condition########################################"
            self.cur = mdb.connect("sqlit3_LOCAL.db")
            cursore=self.cur.execute("SELECT Password from User_authentication where password = (%s)", [old_pwd])
            passwcorrect = cursore.fetchone()
            print'########passwcorrect',passwcorrect
            if passwcorrect is None:
                print 'passwcorrect',passwcorrect
...
##### snehali  new code 
    ...
    def createResumeSubmitTable(self):
        ##self.cur.execute("DROP TABLE Submit_resume")
        try:
            self.cur.execute('''CREATE TABLE IF NOT EXISTS Submit_resume
            (date TEXT,
            resumestatus TEXT,
            Statusofresume TEXT,
            RequirementID TEXT,
    ...
    def createUser_AuthenticationTable(self):
        ##self.cur.execute("DROP TABLE Submit_resume")
        try:
            self.cur.execute('''CREATE TABLE IF NOT EXISTS User_authentication
            (department TEXT,
            mobilenumber TEXT,
            lastname TEXT,            
            firstname TEXT,
    ...
    def createrequirestatuschangeTable(self):
        ##self.cur.execute("DROP TABLE Submit_resume")
        try:
            self.cur.execute('''CREATE TABLE IF NOT EXISTS requirestatuschange
            (Reqid TEXT ,
            oldstatus TEXT,
            newstatus TEXT,            
            Date BLOB,
    ...
    def createstatuschangeTable(self):
        ##self.cur.execute("DROP TABLE Submit_resume")
        try:
            self.cur.execute('''CREATE TABLE IF NOT EXISTS statuschange
            (newstatusofresume TEXT ,
            oldstatusofresume TEXT,
            Date BLOB,          
            Time BLOB,
            Newstatus TEXT,
            Oldstatus TEXT,
...
## end snehali
    ...
    def getResumeDetailById(self,mobNo):
        listOfSearchResult = []
        cursore = self.cur.execute("SELECT resumestatus,Statusofresume,RequirementID,Client,name,Mobile_Number,Email,Date_of_birth,skills,CTC,ECTC,Notice_Period,currentlocation,locationofInterest,PANCARD,Yearsofexperience,Submit_SOurce,Note,DateofSubmission,myResume_File from Submit_resume where \
...
##        
        if (Reqid != ""):
           # print "HELLOOOOOOOOOOOOOOOOOOOOOOOOO *************\n"
            ...
            cursor = self.cur.execute("SELECT resumestatus,Statusofresume,RequirementID,Client,name,Mobile_Number,Email,Date_of_birth,skills,CTC,ECTC,Notice_Period,currentlocation,locationofInterest,PANCARD,Yearsofexperience,Submit_SOurce,Note,DateofSubmission,myResume_File from Submit_resume")
...
##            self.cur.close()
     ...
     #db for status of resume and requirement id by akshay
    
    def getResumedataById11(self,Reqid,statusofresume123):
        print " first time reqid============",Reqid
        print "my akshay",statusofresume123
        
        listOfSearchResult=[]
##        
        if (Reqid != "" and statusofresume123!=""):
           # print "HELLOOOOOOOOOOOOOOOOOOOOOOOOO *************\n"
            ...
            for row in rows:
             ...
             # print "reqid============  ",Reqid
                ...
                if ((row[2].lower().find(Reqid.lower())>=0) and (row[1].lower().find(statusofresume123.lower())>=0)):
...
####        conn = mdb.connect("sqlit3_LOCAL.db")
        ...
        if (d != ""):
           ...
           cursor=self.cur.execute(q)
           conn.commit()
          # cursor = self.cur.execute("DELETE from Submit_resume where Mobile Number = "+'"'+str(d)+'"' )
...
##        return listOfSearchResult
    ...
    def getRequirementID(self):
        ...
        listofRequirementID=[]
        cur=self.cur.execute("Select ReqId from Submit_requir  where Status='open' or Status='Open'")
      ## new snehali code
        rows = self.cur.fetchall()
...
##        print "id ::::::::::::::::::::::"  ,listofRequirementID
    ...
    def getReqID(self):
        ...
        listofRequirementID=[]
        cur=self.cur.execute("Select ReqId from Submit_requir")
...
##        print "id ::::::::::::::::::::::"  ,listofRequirementID
    ...
    def getRequirementID1(self):
        ...
        listofRequirementID=[]
        cur=self.cur.execute("Select RequirementID from Submit_resume")
        ## new snehali code
        rows = self.cur.fetchall()
...
##        print "id ::::::::::::::::::::::"  ,listofRequirementID
    ...
    def getSource(self):
        ...
        listofSource=[]
        cur=self.cur.execute("Select Submit_SOurce from Submit_resume")
...
##        print "id ::::::::::::::::::::::"  ,listofRequirementID
    ...
    def getSkill(self):
        ...
        listofSkill=[]
        cur=self.cur.execute("Select skills from Submit_resume")
        ## new snehali code
        rows = self.cur.fetchall()
...
##        print "id ::::::::::::::::::::::"  ,listofRequirementID
    ...
    def getyrofexp(self):
        ...
        year=[]
        cur=self.cur.execute("Select Yearsofexperience from Submit_resume")
...
##        print "id ::::::::::::::::::::::"  ,listofRequirementID
    ...
    def getBDPOC(self):
        ...
        listofBDPOC=[]
        cur=self.cur.execute("Select BDPOC from Submit_requir")
...
##        print "id ::::::::::::::::::::::"  ,listofRequirementID
    ...
    def getClient(self):
        ...
        listofClient=[]
        cur=self.cur.execute("Select Client from Submit_requir")
...
## new snehali code
    ...
    def getClient1(self):
        ...
        listofClient=[]
        cur=self.cur.execute("Select Client from Submit_resume")
        print "values=",cur
...
####            date=time.strftime("%x")
            ...
            self.cur.execute("INSERT INTO Submit_resume (resumestatus,Statusofresume,RequirementID,Client,name,Mobile_Number,skills,Date_of_birth,ECTC,Notice_Period,CTC,Email,myResume_File,currentlocation,locationofInterest,PANCARD,Yearsofexperience,Note,Submit_Source,DateofSubmission) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (resumestatus,Statusofresume,RequirementID,Client,name,Mobile_Number,skills,Date_of_birth,ECTC,Notice_Period,CTC,Email,tempFilepath,CURRENT_LOCATION,lOCATION_OF_INTEREST,PANCARD_NO,Yearsofexperience,Note,Submit_Source,date))
...
## snehali new code 
    def createRequireSubmitTable(self):
        try:
            self.cur.execute('''CREATE TABLE Submit_requir
            (Opendate TEXT,
            ReqId BLOB,
            Client TEXT,
            Primary_Skill TEXT,
...
##        self.cur = mdb.connect("sqlit3_LOCAL.db")
        listOfSearchResult = []
        cursor = self.cur.execute("SELECT Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note  from Submit_requir where \
                                        ReqId = "+'"'+str(ReqId)+'"' )
...
##        self.cur.close()
    ...
    def getRequireDetailById1(self,ReqId):
        listOfSearchResult=[]
        cursor = self.cur.execute("SELECT Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note  from Submit_requir" )#where ReqId = '"+str(RequirementID)+"'")
        ## new snehali code
        rows = self.cur.fetchall()
...
##        return listOfSearchResult
    ...
    def setRequireDetail(self,Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note):
        ...
        #try:
        cursor= self.cur.execute("INSERT INTO Submit_requir (Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                  (Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note))
        print cursor
        
        # snehali commentd
##        self.cur.commit()
     #### end of sneha comment
...
##        cursor =  self.cur.execute("SELECT name,Mobile_Number,skills,Date_of_birth,ECTC,Notice_Period,\
##        CTC,Email,myResume_File from Submit_resume")
##        cursor =  self.cur.execute("SELECT name,Mobile_Number,skills,Date_of_birth,ECTC,Notice_Period,CTC,Email,myResume_File from Submit_resume")
##        for row in cursor:
...
############## Client
            ...
            print "client===================="
            cursor = self.cur.execute("SELECT Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note  from Submit_requir" )# where Client = '"+str(Client)+"'")
...
##############BY Status
            ...
            print "status===================="
            cursor = self.cur.execute("SELECT Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note  from Submit_requir " )#where Status = '"+str(Status)+"'")#where \
...
########### By ReqId and Client
            ...
            #query to fetch all details for requirement id
            print "req & client===================="
            cursor = self.cur.execute("SELECT Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note  from Submit_requir " )#where ReqId = '"+str(RequirementID)+"'and Client= '"+str(Client)+"'")
## new snehali code
            rows = self.cur.fetchall()
            for row in rows:
...
##########By ReqId and Status             
            ...
            #query to fetch all details for requirement id
            print "req & status===================="
            cursor = self.cur.execute("SELECT Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note  from Submit_requir" )# where ReqId = '"+str(RequirementID)+"'and Status= '"+str(Status)+"'")
## new snehali code
            rows = self.cur.fetchall()
            for row in rows:
...
##            self.cur.close()
        ...
        ##########By Client and Status & Req id 
            ...
            #query to fetch all details for requirement id
            print "all three===================="
            cursor = self.cur.execute("SELECT Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note  from Submit_requir " )#ReqId = '"+str(RequirementID)+"'and Status= '"+str(Status)+" 'and Client= '"+str(Client)+"'")
...
##            q = 'select Opendate from Submit_requir where ReqId = "'+RequirementID+'" and Status = "'+Status+'" and Client = "'+Client+'"'
           # print "QUERY STRING--------------------------------",q
##            con = mdb.connect('sqlit3_LOCAL.db')
##            cur = con.execute(q)
##            con.commit()
##            self.cur.execute(q)
##            self.cur.commit()
...
################ BY ONLY NAME
##        if (Name != "" and  skills=="" and RequirementID=="" and Client=="" and Yearsofexperience=="" ):
##            cursor = self.cur.execute("SELECT resumestatus,Statusofresume,RequirementID,Client,name,Mobile_Number,Email,Date_of_birth,skills,CTC,ECTC,Notice_Period,currentlocation,locationofInterest,PANCARD,Yearsofexperience,Submit_SOurce,Note,DateofSubmission,myResume_File from Submit_resume ")#where \
##                                        #name = "+'"'+str(name)+'"' )
##            for row in cursor:
...
##
##            else:
##                cursor = self.cur.execute("SELECT Statusofresume,RequirementID,Client,name,Mobile_Number,Email,Date_of_birth,skills,CTC,ECTC,Notice_Period,currentlocation,locationofInterest,PANCARD,Yearsofexperience,Submit_SOurce,Note,DateofSubmission,myResume_File from Submit_resume where skills = "+'"'+str(skills)+'"' )
##
##                abc=cursor.fetchone()
##                if (abc==None):
...
##                    self.cur.close()
    ...
    def getidbyBDPOC(self,Name):
        print "BDPOC============",Name
        listOfBDPOCResult=[]
##        cursor = self.cur.execute("SELECT RequirementID from Submit_resume where Submit_SOurce = '"+str(source)+"'")
...
##      
        if (Name != ""):
            ...
            cursor = self.cur.execute("SELECT Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note from Submit_requir where status= 'Open' or 'open' ")
...
##            self.cur.close()
    ...
    def searchAll(self,Value):
            ...
            listOfSearchResult = []
##            self.cur = mdb.connect("sqlit3_LOCAL.db")
            cur=self.cur.execute("SELECT Tag_value from User_authentication where username = (%s)", [Value])
...
##            listOfSearchResult = []
##            self.cur = mdb.connect("sqlit3_LOCAL.db")
##            cursor = self.cur.execute("SELECT RequirementID,Client,name,Mobile_Number,Email,Date_of_birth,skills,CTC,ECTC,Notice_Period,currentlocation,locationofInterest,PANCARD,Yearsofexperience,Submit_SOurce,Note,DateofSubmission,myResume_File from Submit_resume ")
##            print "sdfbhskv oshnvo nsovjosrfvsrvj",cursor
##            for row in cursor:
...
##            return listOfSearchResult
    def searchRequirement(self):
            listOfSearchResult = []
##            self.cur = mdb.connect("rmtdb")
            cursor = self.cur.execute("SELECT Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note from Submit_requir")
            print "sdfbhskv oshnvo nsovjosrfvsrvj",cursor
## new snehali code
            rows = self.cur.fetchall()
            for row in rows:
...
##        conn.commit()
    ...
    def searchopenstateresumes(self,state):
        listofresults=[]
        cursor = self.cur.execute("SELECT Opendate,ReqId,Client,Primary_Skill,noOfPosition,openPosition,Minimumexp,location,BDPOC,Status,Description,Note from Submit_requir where Status='open' or Status='Open'")
## new snehali code
        rows = self.cur.fetchall()
        for row in rows:
...
##            print "ReqId in db query:", RequirementID
           ...
           self.cur.execute("INSERT INTO statuschange (newstatusofresume,oldstatusofresume,Date,Time,Newstatus,Oldstatus,Requirement_Id,Mobile_number) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)", (newstatusofresume,oldstatusofresume,Date,Time,newstatus,oldstatus,RequirementID,Mobile_Number))
...
##           self.cur.commit()
    ...
    def setstatuschange1(self,RequirementID,oldstatus,newstatus,date,time):
        ...
        self.cur.execute("INSERT INTO requirestatuschange (Reqid,oldstatus,newstatus,Date,Time) VALUES (%s,%s,%s,%s,%s)",(RequirementID,oldstatus,newstatus,date,time))
        print "done inserting"
##        self.cur.commit()
####            
##        except:
...
#######################################lavita##entire code starts here#######################################################
    def createSignIn_Up(self):
        try:
             self.cur.execute('''CREATE TABLE User_authentication
        (
    ...
    def newPassword(self,username,oldpassword,newpassword):
        ...
        try:
            ...
            for row in rows:
                ...
                if (str(row[0])== str(username) and str(row[1])== str(password)):
                    flag = "true"
                    self.cur.execute("UPDATE  User_authentication SET password=%s WHERE password = %s",(newpassword,password))
##                    self.cur.commit()
                else:
                    continue
                return flag
...
# new lavita code 
    def signup1(self,department,mobilenumber,lastname,firstname, Tag_value,username,password):
        
        print "=============",username, password,firstname,lastname,mobilenumber

        try:
            
            flag=""
####            self.createSignIn_Up()
##            self.createUser_AuthenticationTable()
            
            if(len(mobilenumber)!=10):
                flag="mobile number should be of 10 or 11 digits"
                return flag
            else:
                 flag="thank you for signup"
                 
##                 self.cur.execute("INSERT INTO  User_authentication VALUES(%s,%s,%s,%s,%s,%s,%s)",
##                              (department,mobilenumber,lastname,firstname,Tag_value,username,password))

                 self.cur.execute("INSERT INTO  User_authentication (department,mobilenumber,lastname,firstname,Tag_value,username,password) VALUES(%s,%s,%s,%s,%s,%s,%s)",
                              (department,mobilenumber,lastname,firstname,Tag_value,username,password))
...
# old 
####    def signup1(self,department,mobilenumber,lastname,firstname,username,password):
####        Tag_value=1
####        print "=============",username, password,firstname,lastname,mobilenumber
####
####        try:
...
#########################Lavita's code ends here################################################################################

#code by akshay for stats,yearofexp and moblno for autocomplete        
    def getautostats(self):
        ...
        rows = self.cur.fetchall()
        for row in rows:
              listOfSearchResult.append(row[8])
##            self.cur.close()
        print listOfSearchResult
        print "list of data skills !!!!!!!!!!!!!::",listOfSearchResult
    ...
    def getautoexp(self):
        ...
        rows = self.cur.fetchall()
        for row in rows:
              listOfSearchResult.append(row[15])
##            self.cur.close()
        print listOfSearchResult
        print "list of data of yearofexp!!!!!!!!!!!!!::",listOfSearchResult
         
        return listOfSearchResult
    
    def getautombno(self):
     ...
     ## new snehali code
        rows = self.cur.fetchall()
        for row in rows:
              listOfSearchResult.append(row[5])
##            self.cur.close()
        print listOfSearchResult
        print "list of data of mbno !!!!!!!!!!!!!::",listOfSearchResult
