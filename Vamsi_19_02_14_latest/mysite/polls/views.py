...
##     DB.createstatuschangeTable()
##     DB.createRequireSubmitTable()
     ## if uncommented below line,  throws warning ....
     # need to debug ... issue of existing tableeeeeeeeee
     
####     DB.createUser_AuthenticationTable()
####     DB.createrequirestatuschangeTable()
####     DB.createstatuschangeTable()
     ###
...
def changep(request):
    ...
    DB=DataBase()
    DB.newPassword(username,password,newpassword)
    ReqId="newpassword="+newpassword
    msg = "Your password is changed successfully your new password is: "+ReqId
...
def signupdata(request):
    ...
    confirmpassword=request.POST["confirmpassword"]
     ...
     ### lavita ERRRRRRORRRRRRRRRRRRRRRRRRR for tag
    Tag_value=1
##    Tag_value=request.POST["Tag_value"]
    ...
    flag=DB.signup1(department,mobilenumber,lastname,firstname,Tag_value,username,password)
    return render(request,'signup.html',{'flag':flag})
...
####    DB=DataBase()
####    print "entering to database"
####    flag=DB.signup1(department,mobilenumber,lastname,firstname,username,password)
####    return render(request,'signup.html',{'flag':flag})
...
#####################
@csrf_exempt
def ReportManagementfile(request):
   # Date1 = request.POST["Date1"]
    ...
    return render_to_response('reportmanagement.html',{'list':list1,'getSource':list2,'getClient':list3})
...
def requiresub(request):
    ...
    getBDPOC.append("ravi.trivedi@brisa-tech.com")
    list2=set(getBDPOC)
...
##    Date2 = request.POST["datepicker1"]
 ...
 #   getSource=dB.getSource()
    ...
    getyr=dB.getyrofexp()
    list5=set(getyr)
    return render_to_response('Search.html',{'statusofresume123':statusofresume1,'getSource':list2,'getClient':list3,"msgRed":"","msgBlue":"search click","msgGreen":"",'value':Value2,'getSource':list2})
...
def searchResult(request):
  ...
  #  skills = request.POST["skills"]
   ...
   # history=request.POST["history"]
    ...
    flag22=0
    RequirementID=request.POST["ReqId"]
    statusofresume123=request.POST["statusofresume123"]
    Client = request.POST["Client"]
...
##         listofid=dB.getidbyBDPOC(Name)
    ...
    ##### requirement id and status of resume by akshay!!!
    if (RequirementID!="" and statusofresume123!="" ):
     ...
     listOfSearchResult=db.getResumedataById11(RequirementID,statusofresume123)
     print "\nlistofdata",listOfSearchResult

     return render_to_response('Search.html',{'requirementids':list1,'client':Client,'reqid':RequirementID,'sres':statusofresume123,'statusofresume123':statusofresume1,'Date1':Date1,'Date2':Date2,'getClient':list3,'getSource':list2,"msgRed":"","msgBlue":"","msgGreen":"In search result",'listSearchResult':listOfSearchResult})
     ##end of akshay code
  ##### date1 !!!
...
##                        flag9=0
    ...
    if(flag7==1):
         print "\n huh"
         if (listofdata!=" "):
           return render_to_response('Search.html',{'skil':only_skills,'ye':yrexp,'Date1':Date1,'Date2':Date2,'getClient':list8,'getSource':list2,"msgRed":"","msgBlue":"","msgGreen":"In search result",'listSearchResult':listofdata })
         else:
    ...
    # new code by gangadhar
    if (flag6==1):
        return render_to_response('Search.html',{'skil':only_skills,'Date1':Date1,'Date2':Date2,'getClient':list3,'getSource':list2,"msgRed":"","msgBlue":"","msgGreen":"In search result",'listSearchResult':listOfSearchResult})

    if (flag22==1):
        return render_to_response('Search.html',{'mob':mobile,'Date1':Date1,'Date2':Date2,'getClient':list3,'getSource':list2,"msgRed":"","msgBlue":"","msgGreen":"In search result",'listSearchResult':listOfSearchResult})
    ...
    if(flag3==1):
        if(not RequirementID):
          ...
          return render_to_response('Search.html',{'requirementids':list1,'client':Client,'reqid':RequirementID,'Date1':Date1,'Date2':Date2,'getClient':list3,'getSource':list2,"msgRed":"","msgBlue":"","msgGreen":"In search result",'listSearchResult':listOfSearchResult})
    ...
    if (flag5==1):
     if( not Client or not skills):
       ...
       return render_to_response('Search.html',{'msg5':msg,'Date1':Date1,'Date2':Date2,'getClient':list3,'getSource':list2,"msgRed":"","msgBlue":"","msgGreen":"In search result"})
        ...
        #print "abc"
       return render_to_response('Search.html',{'skil':only_skills,'client':Client,'Date1':Date1,'Date2':Date2,'getClient':list3,'getSource':list2,"msgRed":"","msgBlue":"","msgGreen":"In search result",'listSearchResult':listOfSearchResult})
    ...
    if(flag4==1):
      if( not Name or not Date1 or not Date2):
        ...
        return render_to_response('Search.html',{'name':Name,'Date1':Date1,'Date2':Date2,'getClient':list3,'getSource':list2,"msgRed":"","msgBlue":"","msgGreen":"In search result",'listSearchResult':listOfSearchResult})
    else:
...
#Edited by Gangadhar
    if( not RequirementID and not Status and not Client):
       ...
       return render_to_response('RequirementManagement.html',{'requirementids':list4,'client':Client,'rid':RequirementID,'stat':Status,'getSource':list2,'list':list1,'getClient':list3,"msgRed":"","msgBlue":"","msgGreen":"In search result",'listSearchResult':listOfSearchResult})
...
##                        flag9=0
    ...
    return render_to_response('Search.html',{'statusofresume123':statusofresume1,'getSource':list2,'getClient':list3,"msgRed":"","msgBlue":"","msgGreen":"In search result",'listSearchResult':listOfSearchResult})
...
##
##       return render_to_response('RequirementManagement.html',{"msg":msg,
             return render_to_response('reportmanagement.html',{'ids':uniqueid1,'dates':datelist,'count':count2,'Date1':Date1,'status':status1,'Date2':Date2})
      else:
             return render_to_response('reportmanagement.html',{'ids':uniqueid1,'dates':datelist,'count':count2,'Date1':Date1,'status':status1})
...
#### corrected display_report removed bugs ..
...
##              snehali=snehali+i[8:]
              i=snehali
              tmp2=''
              print "snehali dataaaaa",snehali
              tmp=snehali
                   ...
                   for k in j:
                       ...
                       if(flag==0):
                                   ...
                                   if(lengthofi>1):
                                       ...
                                       combine.insert(index,count2[s][m])
                                       print "s in loop:",s
                                       print "count2[s][m]",s,m,count2[s][m]
##                                       newind=newind+1
                                       print "newind=",newind
                                       ...
                                       print "3 ...index",index
                                       combine.insert(index,count2[s][z])
                                       print "count2[s][z]",s,z,count2[s][z]
...
##      if(flagerror==1):
##                return render_to_response('RequirementManagement.html',{"msgRed":"Enter two different dates"})
...
## old 
@csrf_exempt
def before_30_june_display_report(request):
...
#######  JSON  OBJ  ######
       ...
       if (status=="DSR"):
		...
		print "\n last"
                ...
                return render_to_response('reportmanagement.html', context_instance=RequestContext(request))
...
########################Lavita's code starts here##################################################################################
    dB=userinfo()
    flag= dB.submitSignIn(username,password)
    if flag!="true":
        return  render_to_response('loginpage.html',{'wrong_user':'Please provide correct username/password'})
    else:
        ...
        if tag_value==0:
            return  render_to_response('loginpage.html',{'statusofresume123':statusofresume1,'wrong_user':'Please provide correct username/password'}) ##Please redirect to same login page with a message Username/password is wrong##
        elif tag_value==1:
            return render_to_response('Search.html',{'statusofresume123':statusofresume1,'getSource':list2,'getClient':list3,"msgRed":"","msgBlue":"search click","msgGreen":"",'value':Value,'getSource':list2})
       # return render(request,'resumeSubmit.html',{'getClient':list3,'status123':status1,'statusofresume123':statusofresume1,'getSource':list2})
            ...
            return render_to_response('loginpage.html',{'statusofresume123':statusofresume1,'wrong_password':'The username or password you entered is incorrect ?'}) ##Please redirect to same login page with a message Password is wrong##
...
####################################################################################################################
#########################
##akshay autocomplete code
@csrf_exempt
def autoskills(request):
        print "code for autoskils"
       	...
       	if(request.POST.has_key('client_response')):
                ...
                reqautostats1=[]
                dB=DataBase()
                reqautostats = dB.getautostats()
                items = [ 'France', 'Italy', 'Malta', 'England', 'Australia', 'Spain', 'Scotland' ];
                for j in reqautostats:
                     
                     if j not in reqautostats1:
                          reqautostats1.append(j)
                          
                print "\n value",reqautostats1

                print "\njson list"
  
                return HttpResponse(json.dumps(reqautostats1), content_type='application/javascript')
        else:
                return render_to_response('Search.html', context_instance=RequestContext(request))
@csrf_exempt          
def autoyearexp1(request):
        print "code for yearofexp"
       	...
       	if(request.POST.has_key('client_response')):
                ...
                reqyearofexp1=[]
                dB=DataBase()
                reqyearofexp = dB.getautoexp()
                items = [ 'France', 'Italy', 'Malta', 'England', 'Australia', 'Spain', 'Scotland' ];
                for j in reqyearofexp:
                     
                     if j not in reqyearofexp1:
                          reqyearofexp1.append(j)
                          
                print "\n value",reqyearofexp1

                print "\njson list"
  
                return HttpResponse(json.dumps(reqyearofexp1), content_type='application/javascript')
        else:
                return render_to_response('Search.html', context_instance=RequestContext(request))
@csrf_exempt
def autombno(request):
        print "code for mbno"
       	...
       	if(request.POST.has_key('client_response')):
                ...
                reqautombno1=[]
                dB=DataBase()
                reqautombno = dB.getautombno()
                items = [ 'France', 'Italy', 'Malta', 'England', 'Australia', 'Spain', 'Scotland' ];
                for j in reqautombno:
                     
                     if j not in reqautombno1:
                          reqautombno1.append(j)
                          
                print "\n value",reqautombno1

                print "\njson list"
  
                return HttpResponse(json.dumps(reqautombno1), content_type='application/javascript')
        else:
                return render_to_response('Search.html', context_instance=RequestContext(request))
